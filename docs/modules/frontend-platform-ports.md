...
| `useModuleSeo()` / `ModuleSeoOptions` | Canonical-, Robots-, OpenGraph-, Twitter- und strukturierte Metadaten über die bestehende Host-SEO-Runtime setzen | im Nuxt-Setup auf Client und Server; erzeugt keine eigene Head-Runtime | entspricht dem bestehenden `usePageSeo`-Fehlerverhalten | stabil ab 1.4 |
