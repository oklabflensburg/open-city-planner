...
| `useMapSelectionPort()` / `MapSelectionPort` | Auswahl über `{ type, id }` lesen, setzen, sichtbar machen oder löschen | im Nuxt-/Pinia-Kontext; `reveal` ändert nur die vorhandene Host-Auswahlfläche | leere Typen oder IDs werfen synchron; unbekannte Typen bleiben ohne Host-Präsentation | stabil ab 1.3 |
