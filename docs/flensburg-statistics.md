...
Der Statistikimport setzt die Gemeinde und alle 13 Stadtteile in `analysis_areas`
voraus. Auf einer neuen Installation muss das externe `analysis-areas`-Modul
aktiviert sein und diese Ressourcen über seinen dokumentierten Betriebspfad
bereitgestellt haben. Fehlen sie, endet der Lauf mit `FAILED`, ohne vorhandene
Statistikdaten zu verändern.
