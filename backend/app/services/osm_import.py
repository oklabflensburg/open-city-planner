...
SNAPSHOT_TAGS = (
    "name", "shop", "amenity", "office", "craft", "tourism", "brand", "operator",
    "opening_hours", "website", "phone", "addr:street", "addr:housenumber",
    "addr:postcode", "addr:city", "level", "building", "building:levels", "disused",
    "disused:shop", "abandoned", "abandoned:shop",
)
...
async def create_polygon_from_osm(
    ...
    source = (
        await session.execute(SOURCE_SQL, {"osm_type": payload.osm_type, "osm_id": payload.osm_id})
    ).mappings().first()
    if source is None:
    ...
    existing = (
        await session.execute(EXISTING_SQL, {
            "osm_type": payload.osm_type, "osm_id": payload.osm_id, "floor": floor,
        })
    ).mappings().first()
    if existing:
    ...
    logger.info(
        ...
        polygon.uuid, payload.osm_type, payload.osm_id, user_id,
    )
    return OsmPolygonImportRead(
        id=str(polygon.uuid), slug=polygon.slug, geometry_source=geometry_source,
        source_osm_type=payload.osm_type, source_osm_id=payload.osm_id,
