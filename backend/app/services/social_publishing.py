...
from app.services.social_policy import event_is_enabled, get_social_settings
...
async def _event_context(
    ...
    if area is None:
        raise MastodonError("Das zu veröffentlichende Gebiet existiert nicht mehr.", retryable=False)
    return (
        render_area_post(
            area,
            event.event_type,
...
def _audit(event: SocialPublicationOutbox, action: str) -> AdminAuditLog:
    ...
    return AdminAuditLog(
        action=action,
        resource_type="ANALYSIS_AREA" if event.resource_type == "ANALYSIS_AREA" else "SYSTEM",
        resource_id=event.resource_id if event.resource_type == "ANALYSIS_AREA" else None,
    ...
    *,
    ...
    for _ in range(limit):
        event = await session.scalar(
            select(SocialPublicationOutbox).where(
                SocialPublicationOutbox.event_type.in_(policy.enabled_events or []),
        ...
        try:
            ...
            text, area = await _event_context(
                session,
                event,
                settings,
            ...
            target = screenshot_target(event, area, settings, policy)
