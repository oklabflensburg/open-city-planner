<template>
  ...
  >
    ...
    <Card class="p-5 sm:p-6">
      <div class="grid gap-4 lg:grid-cols-2 xl:grid-cols-[minmax(15rem,1fr)_15rem_15rem_11rem_11rem_8rem]">
        <label>
          <span class="field-label">Auditlog durchsuchen</span>
          <span class="relative block"><Search class="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-500" /><input v-model="filters.search" type="search" class="field-input pl-10" placeholder="Aktion, Name oder E-Mail" autocomplete="off"></span>
        </label>
        <label><span class="field-label">Aktion</span><select v-model="filters.action" class="field-input" @change="filterChanged"><option value="">Alle Aktionen</option><option v-for="action in availableActions" :key="action" :value="action">{{ auditActionLabel(action) }}</option></select></label>
        <label><span class="field-label">Ausgeführt von</span><select v-model="filters.userId" class="field-input" @change="filterChanged"><option value="">Alle Benutzer</option><option v-for="actor in actors" :key="actor.id" :value="actor.id">{{ actorName(actor) }}</option></select></label>
        <label><span class="field-label">Von</span><input v-model="filters.dateFrom" type="date" class="field-input" @change="filterChanged"></label>
        <label><span class="field-label">Bis</span><input v-model="filters.dateTo" type="date" class="field-input" @change="filterChanged"></label>
        <label><span class="field-label">Pro Seite</span><select v-model.number="filters.pageSize" class="field-input" @change="filterChanged"><option :value="25">25</option><option :value="50">50</option><option :value="100">100</option></select></label>
      </div>
    </Card>
