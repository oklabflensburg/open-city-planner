<template>
  <Card id="benachrichtigungen" class="p-5 sm:p-7">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div class="min-w-0"><p class="civic-kicker">Benachrichtigungen</p><h2 class="mt-1 text-lg font-black text-slate-950">Zustellwege festlegen</h2><p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">In-App-Hinweise und optionale E-Mails können unabhängig voneinander gesteuert werden.</p></div>
    ...
    <div v-else class="mt-5 min-w-0" :aria-busy="state === 'saving'">
      <div class="grid grid-cols-[minmax(0,1fr)_3.75rem_3.75rem] items-end gap-1 border-b border-slate-200 pb-3 text-center text-[10px] font-bold uppercase leading-tight tracking-wide text-slate-500 sm:grid-cols-[minmax(0,1fr)_5.5rem_5.5rem] sm:gap-2 sm:text-xs">
        <span class="text-left">Kategorie</span><span><span class="sm:hidden">App</span><span class="hidden sm:inline">Im Stadtplaner</span></span><span>E-Mail</span>
      </div>
      <div class="grid grid-cols-[minmax(0,1fr)_3.75rem_3.75rem] items-center gap-1 border-b border-slate-100 py-3 sm:grid-cols-[minmax(0,1fr)_5.5rem_5.5rem] sm:gap-2">
        <span class="min-w-0 pr-2 text-sm font-bold leading-5 text-slate-900">Alle optionalen Benachrichtigungen</span>
      ...
      <div v-for="row in rows" :key="row.label" class="grid grid-cols-[minmax(0,1fr)_3.75rem_3.75rem] items-center gap-1 border-b border-slate-100 py-3 sm:grid-cols-[minmax(0,1fr)_5.5rem_5.5rem] sm:gap-2">
        <span class="min-w-0 pr-2"><span class="block text-sm font-semibold leading-5 text-slate-900">{{ row.label }}</span><span class="mt-0.5 block text-xs leading-5 text-slate-500">{{ row.description }}</span></span>
      ...
      <label class="mt-6 flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-4">
        <input class="mt-0.5 size-5 shrink-0 accent-[#0b8190]" type="checkbox" :checked="store.preferences.newsletter_enabled" @change="toggle('newsletter_enabled', $event)">
        <span class="min-w-0"><strong class="block text-sm text-slate-950">Newsletter</strong><span class="mt-1 block text-xs leading-5 text-slate-600">Neuigkeiten rund um den Stadtplaner und das OK Lab Flensburg per E-Mail erhalten.</span></span>
      </label>
    </div>
