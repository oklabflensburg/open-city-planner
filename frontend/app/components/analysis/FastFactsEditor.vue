<template>
  <section aria-labelledby="metrics-editor-title">
    ...
    <div v-else-if="analytics.managementError" class="mt-3 rounded-xl bg-rose-50 p-3 text-xs text-rose-800">
      ...
      <button class="mt-2 font-bold underline" type="button" @click="analytics.loadManagement()">Erneut versuchen</button>
    </div>
    <form v-else class="mt-3 space-y-3" @submit.prevent="save">
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-1 min-[1440px]:grid-cols-2">
        <label v-for="field in numberFields" :key="field.key" class="block text-xs font-semibold text-slate-700">
          ...
          <span class="relative mt-1 block">
            <input v-model="draft[field.key]" class="min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 pr-7 text-sm font-normal text-slate-900 outline-none focus:border-[#154d73] focus:ring-2 focus:ring-[#154d73]/15" type="number" step="0.01" min="0" :max="field.percent ? 100 : undefined">
      ...
      <label class="block text-xs font-semibold text-slate-700">
        Datenstand
        <input v-model="draft.reference_date" class="mt-1 min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm font-normal text-slate-900 outline-none focus:border-[#154d73] focus:ring-2 focus:ring-[#154d73]/15" type="date">
      </label>
      <label class="block text-xs font-semibold text-slate-700">
        Quelle
        <input v-model="draft.source" class="mt-1 min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm font-normal text-slate-900 outline-none focus:border-[#154d73] focus:ring-2 focus:ring-[#154d73]/15" maxlength="1000">
      </label>
      <label class="block text-xs font-semibold text-slate-700">
        Interner Hinweis
        <textarea v-model="draft.notes" class="mt-1 min-h-24 w-full resize-y rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900 outline-none focus:border-[#154d73] focus:ring-2 focus:ring-[#154d73]/15" maxlength="10000" />
      </label>
      ...
      <div class="flex justify-end gap-2">
        <button class="min-h-11 rounded-lg border border-slate-300 px-3 text-xs font-bold text-slate-700 hover:bg-slate-50" type="button" :disabled="analytics.saving" @click="resetDraft">Zurücksetzen</button>
        <button class="min-h-11 rounded-lg bg-[#154d73] px-4 text-xs font-bold text-white hover:bg-[#0f3f61] disabled:cursor-wait disabled:opacity-60" type="submit" :disabled="analytics.saving">
