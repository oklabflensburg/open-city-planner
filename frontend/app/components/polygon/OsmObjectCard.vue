<template>
  <article :class="compact ? 'mt-3' : 'rounded-lg border border-[#e1e6e8] p-4'">
    ...
    <h3 v-if="object.name" class="mt-1 font-bold text-[#202427]">{{ object.name }}</h3>
    <p class="mt-1 text-sm text-[#4f575c]">{{ osmCategoryLabel(object) }}</p>
    ...
    <AreaExternalLinks
      ...
      class="mt-3"
      :area-name="object.name || 'OpenStreetMap-Objekt'"
...
import { formatOsmAddress, osmCategoryLabel, osmObjectUrl, safeOsmWebsite } from '~/utils/osm'
