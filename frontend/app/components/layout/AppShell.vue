...
function handleResponsiveBreakpoint() {
  updateResponsiveMode()
  if (isDesktop.value) mapStore.closeGisPanel()
}
...
.mobile-map-actions { bottom: calc(env(safe-area-inset-bottom) + 2.25rem); display: grid; max-width: calc(100% - 1rem); grid-template-columns: minmax(0, 1fr); gap: 0.375rem; }
...
@media (min-width: 480px) {
  .mobile-map-actions { width: auto; grid-template-columns: max-content; }
}
