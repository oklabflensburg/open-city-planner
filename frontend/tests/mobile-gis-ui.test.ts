...
describe('mobile GIS interface', () => {
  ...
  it('renders search, filter, analytics and selection once for both sheet and side panel', () => {
    ...
    expect(content).toContain("mapStore.activeGisPanel === 'assistant'")
  ...
  it('keeps embedded filter and analysis summaries in the shared panel scroll flow', () => {
    const content = appFile('components/layout/GisPanelContent.vue')
    ...
    expect(content).toContain("mapStore.activeGisPanel === 'analytics'\" embedded")
  ...
  it('uses one mutually exclusive GIS panel state across mobile and compact layouts', () => {
    ...
    const shell = appFile('components/layout/AppShell.vue')
    expect(store).toContain("export type GisPanel = 'assistant' | 'filter' | 'analytics' | 'selection' | null")
    ...
    expect(shell).toContain("mapStore.openGisPanel('assistant')")
  ...
  it('keeps permanent search on desktop only and opens mobile search from the action bar', () => {
    ...
    expect(shell).toContain("'map-action-active': mapStore.activeGisPanel === 'assistant'")
    expect(shell).toContain(':aria-pressed="mapStore.activeGisPanel === \'assistant\'"')
