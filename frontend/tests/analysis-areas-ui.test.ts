...
describe('hierarchical analysis areas', () => {
  it('contributes three zoom-dependent administrative layers with module-owned click priority', () => {
    ...
    expect(runtime).toContain("id: 'analysis-areas.selection'")
    expect(runtime).toContain('context.selection.registerPresentation(presentation)')
    expect(runtime).toContain('areas.presentSelection(String(selection.featureId))')
    expect(runtime).toContain('context.selection.reveal()')
    expect(runtime).not.toContain('useMapStore()')
    expect(runtime).not.toContain('useMapSelection()')
  ...
  it('uses one shared selection abstraction and responsive sidebars', () => {
    ...
    expect(left).toContain('slot="map.layers"')
    expect(right).toContain('<MapSelectionContent />')
    expect(content).toContain('slot="map.selection"')
    expect(readFileSync(resolve(process.cwd(), 'frontend-modules/analysis-areas/layer/app/components/AnalysisAreaCard.vue'), 'utf8')).toContain('useAnalysisAreasStore()')
